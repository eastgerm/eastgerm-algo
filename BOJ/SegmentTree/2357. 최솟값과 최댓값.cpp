...
vector<long long> minTree, maxTree;
...
long long initMax(vector<long long> &a, vector<long long> &tree, int node, int start, int end) {
    ...
    return tree[node] = max(initMax(a, tree, node * 2, start, (start + end) / 2),
